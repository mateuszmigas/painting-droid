...
import { type RemoveBackgroundModelInfo, useRemoveBackgroundModels } from "@/hooks/useRemoveBackgroundModel";
...
const getDefaultModelOptions = (models: RemoveBackgroundModelInfo[], modelId: string) => {
  const model = models.find((model) => model.id === modelId);
  return (model?.definition.removeBackground.optionsSchema || {}) as Record<string, CustomField>;
};

const getDefaultModelOptionsValues = (models: RemoveBackgroundModelInfo[], modelId: string) =>
...
export const RemoveBackgroundDialog = memo((props: { close: () => void }) => {
  ...
  const onSubmit = (data: z.infer<typeof FormSchema>) => {
    ...
    definition.removeBackground
      .execute(modelId, { ...canvasData.size, data: baseImageData! }, optionsValues, config)
      .then((img) => {
