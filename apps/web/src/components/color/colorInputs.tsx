...
export const ColorInputs = memo(
  (props: { color: RgbaColor; onChange: (color: RgbaColor) => void; className?: string }) => {
    const { color, onChange, className } = props;
    const [hex, setHex] = useState<string>(ColorProcessor.fromRgba(color).toHex());
    return (
      <div className={cn("flex flex-col gap-small", className)}>
        {Object.keys(color).map((key) => {
          ...
          return (
            <div key={rgbaKey} className="flex flex-row items-center gap-big justify-between">
              <Label className="text-xs">{translations.rgba[rgbaKey as keyof typeof translations.rgba]}</Label>
              <NumberInput
                max={max}
                min={0}
                step={step}
        ...
        <div className="flex flex-row items-center gap-big justify-between">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              setHex(value);
              isValidHex(value) && onChange(ColorProcessor.fromHex(value).toRgba());
            }}
            onBlur={(e) => {
              const value = e.target.value;
              !isValidHex(value) && setHex(ColorProcessor.fromRgba(color).toHex());
            }}
          />
        </div>
      </div>
