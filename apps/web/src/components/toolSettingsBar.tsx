...
export const ToolSettingsBar = () => {
  ...
  const schema = (canvasToolsMetadata[selectedToolId]?.settingsSchema ?? {}) as CustomFieldsSchema;

  return (
    <div className="h-10 border-b px-2 flex flex-row gap-small items-center">
      {Object.entries(schema).map(([id, customField]) => {
        return (
          <div key={id} className="flex flex-row items-center gap-small text-xs">
            <div className="whitespace-nowrap ">{customField.name}</div>
            <div className="flex" data-testid={testIds.toolSetting(id)} data-type={customField.type}>
              <ToolSetting
                customField={schema[id]}
                value={settings[id] !== undefined ? settings[id] : customField.defaultValue}
                onChange={(newValue) => {
